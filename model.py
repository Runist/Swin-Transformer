...
def window_partition(x, window_size: int):
    ...
    主要思路是将feature转成 (num_windows*B, window_size*window_size, C)的shape，把需要self-attn计算的window排列到第0维，一次并行的qkv就可以了
    Args:
        x: (B, H, W, C)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # 主要思路是将feature转成 (num_windows*B, Mh*Mw, C)的shape，把需要self-attn计算的window排列到第0维，一次并行的qkv就可以了
        # [batch_size*num_windows, Mh*Mw, total_embed_dim]
        B_, N, C = x.shape

        # qkv(): -> [batch_size*num_windows, Mh*Mw, 3 * total_embed_dim]  3 => q,k,v
